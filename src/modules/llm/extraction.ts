...
function buildSystemPrompt(
  ...
  products: Array<{ id: string; name: string; basePrice: number | null; variants: Array<{ color: string | null; size: string | null; price: number | null }> }>
): string {
...
function generateMockResult(): LlmExtractionResult {
  const mockItems: LlmItem[] = [
    {
...
export async function extractOrderInfo(
  ...
  try {
    ...
    const productList = (products as any[]).map((p: any) => ({
      id: p.id,
      name: p.name,
    ...
    const messages = [
      ...
      { role: 'user' as const, content: userMessages },
    ];

    const responseContent = await callLlmApi(messages);

    try {
      const result = JSON.parse(responseContent) as LlmExtractionResult;
      return result;
    } catch (parseError) {
