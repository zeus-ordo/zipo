...
function parseDraft<T extends { missingFields: string }>(draft: T): T & { missingFields: string[] } {
  return {
    ...draft,
    missingFields: (() => {
      try {
...
const orderDraftRouter = Router();
orderDraftRouter.use(authenticate, requireTenant);
...
orderDraftRouter.get('/', async (req, res) => {
  try {
    ...
    res.json({
      data: drafts.map((d) => parseDraft(d as any)),
      pagination: {
...
orderDraftRouter.get('/:id', async (req, res) => {
  try {
    ...
    res.json(parseDraft(draft as any));
  } catch (error) {
...
orderDraftRouter.patch('/:id', async (req, res) => {
  try {
    ...
    if (summaryForStaff !== undefined) updateData.summaryForStaff = summaryForStaff;
    ...
    res.json(parseDraft(updated as any));
  } catch (error) {
...
orderRouter.use(authenticate, requireTenant);
...
orderRouter.patch('/:id', async (req, res) => {
  try {
    ...
    if (recipientAddress !== undefined) updateData.recipientAddress = recipientAddress;
    ...
    if (paymentLastFiveDigits !== undefined) updateData.paymentLastFiveDigits = paymentLastFiveDigits;
